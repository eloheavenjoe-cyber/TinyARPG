...
export class Room {
  ...
  private renderDoors() {
    for (const door of this.doors) {
      ...
      const stoneDark = 0x6e6e6e;
      const outline = 0x5a5a5a;
      const moss = 0x4a8a3a;
      const vine = 0x3a7a2a;

      // Arch frame: outer rectangle with opening cut out via hole
      g.beginFill(stone);
      g.drawRect(x - pillarW, y - archH, w + pillarW * 2, h + archH);
      ...
      g.lineTo(x - pillarW, y - archH);
      g.lineTo(x + w + pillarW, y - archH);
      g.lineTo(x + w + pillarW, y + h);

      // Arch opening outline
      g.moveTo(x, y + h);
      g.lineTo(x, y);
      g.bezierCurveTo(
        x + w * 0.25, y - archH,
      ...
      for (let ly = y + 20; ly < y + h; ly += 20) {
        g.moveTo(x - pillarW, ly);
        g.lineTo(x, ly);
        g.moveTo(x + w, ly);
        g.lineTo(x + w + pillarW, ly);
      }
      for (let ly = y - archH + 14; ly < y; ly += 14) {
        g.moveTo(x - pillarW, ly);
        g.lineTo(x + w + pillarW, ly);
      }

      // Moss spots
      g.beginFill(moss, 0.8);
      g.drawCircle(x - pillarW + 5, y + h - 5, 4);
      g.drawCircle(x + w + pillarW - 5, y + h - 5, 3);
      g.drawCircle(x - pillarW + 7, y - archH + 6, 3);
      g.drawCircle(x + w + pillarW - 7, y - archH + 8, 2);
      g.endFill();

      // Vines
      g.lineStyle(2, vine, 0.7);
      for (let vi = 0; vi < 2; vi++) {
        const vx = cx + (vi === 0 ? -w * 0.3 : w * 0.3);
        g.moveTo(vx, y - archH);
        g.lineTo(vx, y - archH + 20 + vi * 5);
      }
